...
#include "universities_followers.csv"

using namespace std;

// Constante que se utiliza en la función de hash
...
void generate_random_numbers(int n, int max, const char *universities_followers)
{
    std::ofstream file(universities_followers);
...
void test_insert(HashTable &ht, int n_inserts, int n_tests, const char* numbers_file, const char* out_file, bool overwrite = false){
    ...
    file_out << n_inserts << "," << accumulate(times.begin(), times.end(), 0.0) / times.size() << std::endl;
    file.close();
    file_out.close();
}

void test_search(HashTable &ht, int n_searches, int n_tests, const char* numbers_file, const char* out_file, bool overwrite = false){
    std::ifstream file(numbers_file);
    std::ofstream file_out(out_file, overwrite ? ios::trunc : ios::app);
    ...
    file_out << n_searches << "," << accumulate(times.begin(), times.end(), 0.0) / times.size() << std::endl;
    file.close();
    file_out.close();
}
/*
void test_insert(unordered_map<int, int> &um, int n_inserts, int n_tests, const char* numbers_file, const char* out_file, bool overwrite = false){
    ifstream file(numbers_file);
    ofstream file_out(out_file, overwrite ? ios::trunc : ios::app);
...
int main(int argc, char const *argv[])
{
    ...
    //generate_random_numbers(N, N, "random_numbers.txt");
        ...
        std::cout << "Insertando números aleatorios" << std::endl;
        for(int i = 0; i < 10; i++){
            test_insert(ht_linear, pow(2, 10+i), 1, "random_numbers.txt", "linear_insert.csv", false);
        }
        //test_insert(ht_linear, 1000000, 1, "random_numbers.txt", "linear_insert.csv", false);

        std::cout << "Buscando números aleatorios" << std::endl;
        test_search(ht_linear, 1000000, 1, "random_numbers.txt", "linear_search.csv", false);
